...
### Android Editor APK（已取消）

2026-09-06 決定停止維護平板用自訂 Android Editor，C# 編譯與執行改採遠端環境處理。Godot 4.7.2 官方 Android Editor 與先前自訂 APK 經解包確認，均不含 `libmonosgen`、`GodotSharp` 或 .NET runtime；即使能略過相容性警告開啟含 `C#` feature 的專案，仍無法在 Android 平板上建置或執行 C# 腳本。

先前產生的通用 APK、四架構 SCons 物件、Gradle 輸出，以及為該次建置下載的 Swappy／Perfetto 額外檔案均已從本機清除。本倉庫目前不提供或部署自訂 Android Editor APK；Windows Mono Editor 的建置與部署流程不變。
